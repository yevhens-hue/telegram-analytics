...
def run_analytics_cycle():
    ...
    with get_connection() as conn:
        ...
        c.execute("SELECT app_name, position, category FROM position_history WHERE date = ?", (today,))
        ...
        for app_name, pos, cat in current_apps:
            ...
            # Предыдущая позиция для расчёта роста
            c.execute("SELECT position FROM position_history WHERE app_name = ? AND date = ?", (app_name, yesterday))
            prev_res = c.fetchone()
            prev_pos = prev_res[0] if prev_res else pos
            growth = prev_pos - pos

            # TON метрики
            c.execute("SELECT daily_revenue_ton, daily_active_wallets FROM ton_metrics WHERE app_id = ? AND date = ?", (app_name, today))
            ton_res = c.fetchone()
            ...
            if raw_revenue > 0:
                ...
                revenue = random.uniform(500, 5000) if pos < 10 else random.uniform(10, 200)
                is_mock = 1

            if raw_dau > 0:
                dau = raw_dau
            else:
                dau = random.randint(50000, 500000) if pos < 10 else random.randint(100, 2000)
                is_mock = 1

            # Engagement Rate из статистики каналов
            c.execute("SELECT err FROM channel_stats WHERE app_name = ? AND date = ?", (app_name, today))
            err_res = c.fetchone()
            engagement_rate = err_res[0] if err_res else 0

            # Рекламные расходы
            c.execute("SELECT estimated_budget FROM ad_campaigns WHERE app_name = ? AND date = ?", (app_name, today))
            ad_res = c.fetchone()
            ad_spend = ad_res[0] if ad_res else 0

            # Формула Organic Index
            ...
            # Trend Score
            trend_score = (growth * 12) + (revenue / 50) + (engagement_rate * 5)

            # Market Sentiment (взвешенная комбинация ERR и роста)
